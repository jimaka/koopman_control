...
2. **YAML 合并**：`merge_yaml(args, parser, sys.argv[1:])`（T:680）——读取 `--config` 指定的 YAML，
   ...
   两个边界注意：对 `--key=value` 写法的显式参数会漏判（此时 YAML 会覆盖命令行）；
   YAML 中拼错的键名（未在 parser 中定义）因 `hasattr` 检查被**静默忽略**，不报错——
   "YAML 改了不生效"类问题多源于此；
...
7. **分布式初始化**：`init_distributed(backend)`（T:69）——无 `LOCAL_RANK` 或 `WORLD_SIZE≤1` 返回单进程
   ...
   **CUDA 可用时**先 `torch.cuda.set_device(local_rank)`，再 `dist.init_process_group(init_method="env://")`
   （纯 CPU 的 gloo 路径跳过 `set_device`）；
...
| 配置来源 | 不带 `--config`，全靠 `--` 后透传 CLI 参数 + 默认值 | 总是带 `--config`（默认 `new_v4_dict_input/configs/v4_dt4s_train.yaml`，可被 `CONFIG` 环境变量改指其他文件，如 10step 配置）；`--resume` 由 `RESUME` 环境变量注入；ckpt 根目录可被 `CKPT_DIR` 环境变量覆盖 |
| GPU 数 | 优先 `NPROC` 环境变量 / `--nproc N`，缺省 `nvidia-smi -L` 自动探测；`MASTER_PORT` 环境变量可改端口 | 由环境决定 |
| 职责 | 只做训练 | 训练 → 定位/补齐 best ckpt → 从 YAML 推 pred_len → 自动调 `eval_v4_dict_input.py` 评估 |

注意：`run_v4_dt4s.sh` 的评估段**硬编码** `--data data/koopman_test.npz` 与 `--dt 4.0`，不随 `CONFIG`
变化——若用 `CONFIG` 环境变量切到非 dt=4s 的配置，评估步长会错（pred_len 倒是正确地从 YAML 推断，
优先取显式 `pred_len_max`）。
...
两配置都未设 `noise_std` / `ctrl_noise_std`，正式训练默认关闭噪声注入（仅 smoketest 会条件开启，见 §9）。
...
`_flatten_segments` 会**整段丢弃**长度 `T ≤ data_span` 的航段；`stride` 参数控制段内窗口起点的
步进间隔（默认 1，即逐行滑窗）——两者共同决定有效样本量。
...
- **`__len__`**（T:203）：合法窗口数 M；
- **`__getitem__(index)`**（T:206）：设 k=pred_len、m=model_stride、t₀=`t0_global[index]`——

  | 字段 | 取数方式（slice stop 为右开） | 形状 |
  |---|---|---|
  | `x_t_n` | `states_full[t0]`，当前全状态 | (6,) |
  | `x_seq_n` | `states_full[t0+m : t0+1+k*m : m]`，原始行 t0+m … t0+km，即未来第 1..k 个模型步末状态 | (k,6) |
  | `u_seq_n` | `ctrls_full[t0 : t0+k*m : m]`，原始行 t0 … t0+(k-1)m，即每个模型步起点的控制 | (k,4) |
...
- `num_workers=args.num_workers`（默认 8）、`pin_memory`（有 CUDA 时）、
  `persistent_workers=(num_workers>0)`、`drop_last` 仅 DDP；
...
- **16 个 PIF 字典原子**（`FEATURE_DICT_ATOMS_16`）：二次阻尼项（$u|u|$、$v|v|$、$r|r|$）、耦合项
  ...
  与 $u^2|u|$、$v^2|v|$（即 $|u|^3$、$|v|^3$，**恒非负、不带符号**——变量名 `uuu/vvv` 的误导性正在于此：
  既非 $u^3$ 的形式也无 $u^3$ 的符号），整体对应 MMG 类水动力模型的典型结构；
...
- **初始化**（`reset_parameters`）：encoder/decoder 内所有 `nn.Linear` 做 `kaiming_uniform_(a=√5)` +
  偏置 $\mathcal{U}(\pm 1/\sqrt{fan\_in})$；`A.weight ~ N(0, 0.01²)`、`A.bias=0`（初始近恒等映射，
  训练初期稳定）；`B.weight` 取 xavier(gain=0.1) 小增益；
- **`_self_check_dict`**：用纯 Python 公式逐元素核对 16 个原子（训练启动前 T:821 调用）；
  自检前临时把 `clamp_pif` 置 0、`finally` 中恢复，避免 clamp 掩盖原子公式错误；
...
- **`l_lin` 的目标未 detach**：`target_lat = model.encode(GT)` 保留计算图，梯度同时从目标侧回流
  encoder——encoder 既是"学生"的组件又是"老师"，这是该损失的实际约束方式；
- **`l_xy`/`l_yaw` 条件计算**：仅当 `w_xy>0` 或 `w_yaw>0` 时才执行欧拉积分与位姿损失（权重设 0
  可完全关闭该分支，省算力）；
- **目标位姿口径**：`l_xy`/`l_yaw` 的目标是**数据集记录的真实位姿**
  （`denorm_pose(x_target_seq[:,:,:3])`），只有预测侧来自 `integrate_pose_from_vel` 欧拉积分——
  与 §8 rollout 指标的 GT xy（GT 速度 + 相同初值积分）口径不同，不要混淆；
- 配套工具函数：
...
flowchart TD
    ...
    E -->|否| F
    E1 --> F["epoch 循环 T:941"]
    F --> G["8a 课程更新 T:942<br/>pred_len 变了则重建 loaders<br/>train_sampler.set_epoch(epoch)"]
...
5. `resolve_device`：DDP → `cuda:{local_rank}`（无 CUDA 时落 CPU，对应 gloo 调试路径，DDP 包装也不传
   `device_ids`）；单进程 → `auto`（有 CUDA 用 CUDA）；
...
- **每 epoch 开头**重算 `curriculum_pred_len`，变化则重建 dataloaders（复用旧 stats）；DDP 下随后
  `train_sampler.set_epoch(epoch)`（T:952）——缺了它各 epoch 洗牌顺序相同，是 DDP 正确性的关键点；
...
- 保存策略：每 epoch 覆盖 `koopman_v4_latest.pth`；**最后 30 个 epoch 内每 5 个 epoch**另存
  ...
  注意 latest 先于 best_metric 更新写入，故 **latest 中的 best_metric 滞后一轮**——
  用 latest 续训时恢复的 best_metric 会比实际偏大一轮。
...
2. **YAML 导出**（仅当 `koopman_v4_best.pth` 存在时执行，正常训练首轮 composite 必改善，故实际总会存在）：
   加载 best ckpt（权重优先取 `ema_state_dict`）→ `export_params_to_yaml`（T:694）
   导出 `koopman_v4_best.yaml`，字段含 `normalization.{dyn_mean,dyn_std,ctrl_mean,ctrl_std}`、
   `system_matrices.{A_weight = W_A+I, A_bias, B}`、
   `dictionary.{atoms_16, latent_dim, hidden_dim, input_mode="dict16_only"}`——
...
3. `--no-final-eval` 时跳过；否则用 best ckpt 调 `eval_v4_dict_input.py` 的 `evaluate(...)`
   （`pred_len=pred_len_max`、全量样本、`batch_size=args.batch_size`——注意与 epoch 内验证用
   `val_batch_size` 不同），产物落盘 `log_dir/eval_v4_{ts}/`，8 个文件（逐步指标 CSV、
   `summary.json`、u/v 通道各 3 张图）全部以 `args.run_tag`（默认 `v4_dict_input`）为前缀；
...
键为 `test/L_total … L_stab, test/spec_radius`，按 batch 数算术平均。样本量由独立的
`--val_loss_max_samples` 控制：默认 None = 跟随 `val_max_samples`（512），-1 = 全量（旧行为）；
与 rollout 侧的 `t0_global` 等距子采样是**两个独立口径**（都按 linspace 等距抽取，但作用对象不同），
且只影响 `test/L_*` 日志口径，不参与 best 选择。
...
- 仅 rank0 从训练/测试 npz 各切迷你集（前 2 / 前 1 个航段）写入
  `logs/smoketest_v4_dict_input/mini_{train,test}.npz`，DDP 下写完 `dist.barrier()` 让其他 rank 读到；
- 覆写 args：`train_data`/`test_data` 指向迷你集，`ckpt_dir`/`log_dir` 重定向到
  `logs/smoketest_v4_dict_input/{ckpt,log}`，`epochs=2`、`batch_size=16`、
  `pred_len_start=pred_len_max=4`（`grow_every=999` 冻结课程）、`num_workers=0`；
- 噪声**条件开启**：仅当 `noise_std` / `ctrl_noise_std` ≤ 0 时设为 0.02 / 0.003（用户显式给的
  非零值保留）——目的是保证噪声注入路径被覆盖验证，而非强制改写；
- 跑完整 `train()` 后，rank0 在重定向后的 ckpt 目录校验三件产物：
...
| 函数/类 | 位置 | 功能 |
|---|---|---|
| `DistInfo` | T:57 | DDP 状态值对象（enabled/rank/local_rank/world_size/backend） |
...
| `merge_yaml` | T:680 | YAML 合并（CLI > YAML > 默认；未知键静默忽略） |
| `export_params_to_yaml` | T:694 | 导出 C++ MPC 用 YAML（A+I、统计量） |
| `TrainState` | T:719 | epoch / best_metric 记录 |
