...
| `--noise_std` | `0.0` | 训练时对 t0 归一化 dyn `(u,v,r)` 注入 `N(0,σ)`；**仅 training**。实船/sim→real 建议 `0.02～0.03`，见 [数据增强技术方案](./数据增强技术方案.md) |
| `--ctrl_noise_std` | `0.0` | 训练时对每步归一化控制注入 `N(0,σ)`；**仅 training**。建议 `0.003～0.005` |
