...
| `--scheduler` | `warmrestart` | `warmrestart`=历史默认 `CosineAnnealingWarmRestarts(T_0=cos_T0)`，注意 **T_0=1000 时在常规 epoch 数内 LR 几乎恒定**；`cosine`=`CosineAnnealingLR(T_max=epochs)`（推荐显式退火时使用）；`constant`=恒定 LR |
