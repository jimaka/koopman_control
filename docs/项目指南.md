...
│   └── mpc/controller.py      # MPC 航迹跟踪
├── scripts/                   # 推荐使用的 CLI 入口
...
根目录保留 `train_koopman_v2.py` 等**兼容入口**，内部转发到 `scripts/`。路径常量统一见 `koopman/paths.py`。
...
### 阶段 D2：C++ MPC（LibTorch）

C++ 实现位于 **`cpp/koopman_mpc/`**，与 Python 共用同一 TorchScript rollout（`weights/koopman_rollout.pt`）。

```bash
# 一键：导出权重、编译、rollout 数值验证、MPC 冒烟
...
**注意**：TorchScript 导出时 **固定 `horizon=20`**，C++ 侧 `--horizon` 必须为 20。
