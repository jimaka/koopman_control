...
C++ 实现位于 **`cpp/koopman_mpc/`**：

1. 从 checkpoint 导出 **`weights/koopman_rollout.onnx`**（`export_onnx.py`，PT vs ONNX 误差约 1e-6 量级）；
2. 闭环 MPC 用 **ONNX Runtime** 做 rollout 前向；
3. 优化器为 **自实现 Adam + 前向差分数值梯度**（非 OSQP/Ipopt；与 Python 的 `torch.optim.Adam` + autograd 不同）。
...
**注意**：ONNX 导出时 **固定 `horizon=20`**，C++ 侧 `--horizon` 必须为 20。重新训练后需重新执行 `build.sh` 更新 `.onnx`。

可选：仍可用 `export_torchscript.py` 生成 `koopman_rollout.pt` 作对照，但 **C++ MPC 默认加载 ONNX**。

详见 [`cpp/koopman_mpc/README.md`](../cpp/koopman_mpc/README.md) 与 [`MPC使用指南.md`](./MPC使用指南.md)。
