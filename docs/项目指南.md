...
本文档介绍仓库整体目标、代码结构、数据格式，以及从原始 rosbag 到训练、验证的完整脚本使用流程。快速命令可参考根目录 [`README.md`](../README.md)。仿真/辨识数据切到实船时的优化清单见 [`仿真到实船优化指南.md`](./仿真到实船优化指南.md)；v4 噪声增强见 [`数据增强技术方案.md`](./数据增强技术方案.md)；线性卡尔曼滤波推导与参考实现见 [`卡尔曼滤波推导.md`](./卡尔曼滤波推导.md)。
...
- 模型预测控制（MPC）中的短期航迹推演
- 仿真与实船之间的动力学近似（详见 [仿真到实船优化指南](./仿真到实船优化指南.md)）
...
**Tier-2 物理位姿跟踪（可选）**：设 `w_xy>0` 或 `w_yaw>0` 时启用，跟踪 `(x,y,ψ)`。
位姿由 `decoder(z)→(u,v,r)` 的船体系欧拉积分得到（非线性），在标称轨迹处一阶线性化为
`P_pose ≈ p_free + Φ·U` 后叠加进同一 OSQP，并以 **SQP 外迭代**（`sqp_iters`）修正长程线性化误差。
实船经 motion 桥接时须提供当前全局位姿（`MotionSolveInput.{x,y,psi}`、`has_pose=true`），由桥接层变换到船体系。详见 [潜空间QP-MPC实现.md](./潜空间QP-MPC实现.md)。
...
| 脚本 | 运行位置 | ONNX | 用途 |
|------|----------|:----:|------|
| `cpp/koopman_control/build_mpc_only.sh` | 容器内 | ❌ | 仅 MPC 核心（`-DKOOPMAN_ENABLE_ONNX=OFF`），无 ONNX 下载 |
| `cpp/koopman_mpc/build_in_docker.sh` | 容器内 | ✅ | 全量（权重 + ONNX plant + demo） |
| `cpp/koopman_mpc/build_v4_in_docker.sh` | 宿主机 | ✅ | 启动器（docker exec 进容器全量编译） |
| `cpp/koopman_mpc/build_v4.sh` | 任意 | ✅ | 底层全量构建（导出 latent YAML + ONNX + 编译 + 验证） |

ONNX 下载受限时用 `--ort-tgz <离线包>` 或 `ORT_URL=<镜像>`（脚本内置重试/断点续传）；跨机器复用 checkout 时脚本会自动清理陈旧 `build/` 缓存。
...
| 脚本 | 输入 | 输出 | 备注 |
|------|------|------|------|
| `scripts/sea_trial/run_collection.py` | profile | 开环指令 / 日程 JSON | 全机动采集；见 [数据采集运行指南](./数据采集运行指南.md) |
| `bag_test.py` | rosbag | `koopman_train/val/test.npz` | 15000s 时间轴切分 |
| `split_high_density_bag.py` | rosbag | 同上 | 25200s 高密度方案 |
...
4. 若需重建数据：海试采集 [数据采集运行指南](./数据采集运行指南.md) → §5 阶段 A → `check_dataset` → 训练 → 评估  
5. 航迹跟踪控制：§5 阶段 D（C++ OSQP MPC）与 D2（motion 集成）
6. 调 v3a 时：§5 阶段 B4 + 阶段 E  
7. 仿真/辨识数据切实船：[仿真到实船优化指南](./仿真到实船优化指南.md)  
8. v4 噪声增强参数与验收：[数据增强技术方案](./数据增强技术方案.md)  
9. 状态估计理论参考：[卡尔曼滤波推导](./卡尔曼滤波推导.md)（`koopman.estimation` / `scripts/demo_kalman.py`）
