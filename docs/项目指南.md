...
│   └── mpc/controller.py      # Python MPC 航迹跟踪
├── scripts/                   # CLI 入口（在仓库根目录执行）
│   ├── train_v2.py            # v2/v3/v3a 训练
│   ├── train_v1.py            # v1 训练
│   ├── eval.py                # 评估
│   ├── mpc_track.py           # MPC 闭环仿真
...
> 详细说明、参数表与实测结果见 **[MPC使用指南.md](./MPC使用指南.md)**。
...
2. 与 `scripts/eval.py` 相同，用船体坐标系欧拉积分得到 `(x,y,ψ)`；
3. 最小化相对参考的位姿 / 速度误差 + 控制幅值与平滑惩罚（可分组 `w_du_throttle` / `w_du_rudder`）；
4. 可选 **控制块零阶保持**（`control_hold_steps`）：预测仍用细步 rollout，控制可每 1 s 变一次（v4 默认 hold=10）；
5. 可选 **变化速率硬约束**（`throttle_du_max` / `rudder_du_max`）；
6. 仅施加第 1 个控制，状态前移，重复（MPC 滚动时域）。
...
python3 scripts/mpc_track.py \
    --ckpt checkpoints/koopman_v3a_best.pth \
    --data data/koopman_test.npz --segment 0 --steps 150 \
    --out_dir eval_out/mpc_seg0

# 合成圆周 / 直线参考
python3 scripts/mpc_track.py --ref circle --steps 200 --out_dir eval_out/mpc_circle
python3 scripts/mpc_track.py --ref line --steps 120 --out_dir eval_out/mpc_line

# 调 MPC 权重与求解迭代
python3 scripts/mpc_track.py --segment 0 --w_xy 20 --w_yaw 8 --opt_iters 50
...
**主要参数**（`koopman.mpc.MPCConfig` / CLI）
...
| `control_hold_steps` | 1（代码默认） | 每 N 细步共用一个控制；v4 C++ yaml 默认 10 |
| `throttle_du_max` / `rudder_du_max` | 0 | 块间变化速率硬约束（C++ yaml 默认 15 / 3.5） |
...
### 阶段 D2：C++ MPC（ONNX Runtime）

C++ 实现分为两层：

| 目录 | 作用 |
|------|------|
| **`cpp/koopman_control/`** | 独立控制库：`KoopmanMpcController`、YAML 配置、motion 桥接 |
| **`cpp/koopman_mpc/`** | demo 程序、`build.sh` / `build_v4.sh`、ONNX 权重 |

1. 从 checkpoint 导出 **`weights/koopman_rollout.onnx`**（v4：`export_v4_onnx.py --pred_len 200`；v3：`export_onnx.py`，H=20）；
2. 闭环 MPC 用 **ONNX Runtime** 做 rollout 前向（**一次 Run = H 步图内 rollout**）；
3. 优化器为 **源码内自实现 Adam + 前向差分数值梯度**（**非**独立链接库；链接 ONNX Runtime + yaml-cpp）。
...
MPC 参数：`cpp/koopman_control/config/mpc_config.yaml`（含 `control_hold_steps`、`throttle_du_max` 等）。

**v3 历史路径（H=20）**

```bash
# 一键：安装云端依赖、下载 ORT、导出 ONNX、编译、全流程验证
...
**注意**：ONNX 的 H 在导出时固定；v3 为 **20**，v4 20 s 模型为 **200**。重新训练后需重新导出 `.onnx` 并同步 yaml 中 `horizon`。

可选：仍可用 `export_torchscript.py` 生成 `koopman_rollout.pt` 作对照，但 **C++ MPC 默认加载 ONNX**。
...
#### `scripts/mpc_track.py` / `koopman/mpc/controller.py`

将训练好的 Koopman 模型用作 MPC 内部预测器；`KoopmanMPC.solve` 每步优化推进器序列，`simulate` 执行闭环跟踪。CLI 提供 segment / line / circle 三种参考航迹与结果绘图。
...
5. 航迹跟踪控制：§5 阶段 D（`scripts/mpc_track.py`）与 D2（C++ ONNX）  
