...
flowchart TB
    subgraph L0["⓪ 潜空间权重层"]
        YAML["koopman_v4_latent.yaml<br/>Ā,B,encoder"]
    end
...
潜空间参考由每步 `encode([u_ref,v_ref,r_ref])` 构造（Tier-1；物理位姿跟踪 Tier-2 未实现）。
...
| `new_v4_dict_input/export_v4_encode_weights.py` | 潜空间 YAML 导出 |
