...
int main(int argc, char** argv) {
  ...
  if (!config_path.empty()) {
    try {
      cfg = loadMpcConfigFromYaml(resolvePath(config_path), cfg);
      YAML::Node yaml_node = YAML::LoadFile(resolvePath(config_path));
      if (yaml_node["steps"]) {
        steps = yaml_node["steps"].as<int>();
      }
      ...
      std::cerr << "Error loading YAML config '" << config_path << "': " << e.what() << "\n";
      return 1;
    }
  }
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--smoketest") {
      ...
      std::cout << "Usage: koopman_mpc_cpp [--config config.yaml] [--weights DIR]\n"
                << "       [--ref cpp_test_ref.json] [--steps N] [--horizon H]\n"
                << "       [--opt_control_steps N] [--opt_iters N] [--smoketest]\n";
      return 0;
    }
  }
  ...
  try {
    ...
    cfg = syncHorizonWithOnnx(cfg, model.horizon());
    if (smoketest) {
      cfg.opt_iters = 5;
    ...
    KoopmanMpcController mpc(std::move(model), cfg);
