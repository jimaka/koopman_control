...
- [docs/v4代价函数设计分析.md](docs/v4代价函数设计分析.md) — v4 训练损失与潜空间 MPC QP 目标的量化分解、设计问题与改进优先级
- [docs/v4多步训练代价函数分析.md](docs/v4多步训练代价函数分析.md) — 多步维度专项：逐步损失质量分布、梯度归属、curriculum 量级漂移、ZOH 输入失配
- [docs/多步漂移优化方案.md](docs/多步漂移优化方案.md) — 架构师 + 深度学习专家双视角工程分析与多步预测漂移优化方案（P0–P2）
- [docs/架构重构方案.md](docs/架构重构方案.md) — 架构师视角：现状评估、问题清单（A/B/C 级）、目标架构与分阶段重构计划
- [docs/下一代系统设计方案.md](docs/下一代系统设计方案.md) — 数据 × 深度学习 × 控制三视角全新设计：三层激励采集、耗散参数化网络、无量纲损失栈、QP-MPC 2.0 与功能拆分（含 mermaid 图）
- [docs/koopman模型逐行设计文档.md](docs/koopman模型逐行设计文档.md) — `model_v1_v2.py`、`model_v4_dict_input.py` 与 v4 训练脚本逐行解释、设计动机与两代对照（物理字典、残差参数化、损失栈、curriculum 与 best 选型）
